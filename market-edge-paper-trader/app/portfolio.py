...
class Portfolio:
    def __init__(self, run_mode: str = "backtest"):
    ...
    def _refresh(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT SUM(position_value_pln) FROM trades WHERE status='open' AND run_mode=?",
                (self.run_mode,)
            )
            self.invested_pln = cur.fetchone()[0] or 0.0

            cur.execute(
                "SELECT SUM(pnl_pln) FROM trades WHERE status='closed' AND run_mode=?",
                (self.run_mode,)
            )
            ...
            cur.execute(
                "SELECT COUNT(*) FROM trades WHERE status='open' AND run_mode=?",
                (self.run_mode,)
            )
            self.open_positions = cur.fetchone()[0]

            cur.execute(
                "SELECT COUNT(*) FROM trades WHERE status='closed' AND run_mode=?",
                (self.run_mode,)
            )
    ...
    def _unrealized_pnl(self, cur) -> float:
        cur.execute(
            "SELECT pnl_pln FROM trades WHERE status='open' AND run_mode=?",
            (self.run_mode,)
        )
        return sum(r[0] or 0.0 for r in cur.fetchall())
    ...
    def get_open_tickers(self) -> set[str]:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT DISTINCT ticker FROM trades WHERE status='open' AND run_mode=?",
                (self.run_mode,)
            )
    ...
    def save_snapshot(self, today: date, daily_pnl: float = 0.0):
        ...
        with db_cursor() as cur:
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                str(today), self.run_mode,
                round(self.total_value_pln, 2), round(self.cash_pln, 2),
                round(self.invested_pln, 2), self.open_positions,
                round(daily_pnl, 2),
                round(self.total_value_pln - INITIAL_CAPITAL_PLN, 2),
                round(self.total_return_pct, 4), round(max_dd, 4),
    ...
    def _count_results(self, result: str) -> int:
        ...
        try:
            ...
            if result == "win":
                cur.execute(
                    "SELECT COUNT(*) FROM trades WHERE status='closed' AND pnl_pln > 0 AND run_mode=?",
                    (self.run_mode,)
                )
            else:
                cur.execute(
    ...
    def _calc_max_drawdown(self) -> float:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT total_value_pln FROM portfolio_snapshots WHERE run_mode=? ORDER BY snapshot_date ASC",
                (self.run_mode,)
            )
