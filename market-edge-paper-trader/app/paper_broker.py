...
class PaperBroker:
    def __init__(self, pln_usd_rate: float = PLN_USD_RATE):
        ...
        with db_cursor() as cur:
            cur.execute("""
                ...
                (ticker, strategy, signal_date, score, entry_price, stop_loss,
                 take_profit, atr, rsi, volume_ratio, reason, acted)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    def update_strategy_stats(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT DISTINCT strategy FROM trades WHERE status='closed'")
            ...
            for strat in strategies:
                ...
                cur.execute("""
                    ...
                    (strategy, total_trades, win_trades, loss_trades, total_pnl_pln,
                     avg_win_pln, avg_loss_pln, profit_factor, win_rate, avg_r_multiple,
                     avg_holding_days, last_updated)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
                """, (strat, total, len(wins), len(losses), round(total_pnl, 2),
